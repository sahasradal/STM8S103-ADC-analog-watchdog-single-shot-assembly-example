stm8/
	; Analog watchdog works fine in single shot mode polling, ADC input on PC4 AIN2
	; UART baud 9600 TX on PD5
	; watchdog threshold upper is 500 count and lower threshhold is 100 count
	; issues warning message if count breaches 100 or 500 mark.
	#include "mapping.inc"
...
ADC_INIT:
	...
	bset ADC_AWCRL,#2
	mov ADC_HTRH,#$7D	; high 8 bits of 500 , watchdo high threshold
	mov ADC_HTRL,#$00	; low 2 bits of 500
	mov ADC_LTRH,#$19	; high 8 bits of 100 , watchdog low threshold
	mov ADC_LTRL,#$00	; low 2 bits of 100
...
con_not_finish:
	...
	ld a,ADC_CSR		; copy ADC flag register to A
	and a,#$40			; AND with 0x40 contents of flag register,if set AWD flag set
	ld ADCresult7,a		; store AWD flag in ADCresult7
	ld a,ADC_DRL		; copy ADC lower result register to A ,(buffer2)
	ld ADCresult5,a		; store in ADCresult5
	ld a,ADC_DRH		; copy ADC higher result register to A( ,buffer2)
...
ADC2:	
	...
	ld a,ADCresult7		; laod A with AWD flag in ADCresult7
	cp a,#0				; if flag register is not zero, AWD flag set
	jrne warning		; jump to label warning
	...
	jp delay			; if flag not set jump to delay label
warning:	
	pointerX #warn		; point X to address of string "warning"
	call stringloop		; print via UART 9600
	pointerX #EOL		; pointerX points to string EOL
	call stringloop		; transmit end of line ASCII characters, newline,carriage return etc
...
L1:
	...
	jp ADC_READ			; jump to ADC_READ label to restart  ADC
